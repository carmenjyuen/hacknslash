...
//		_toon.Awake();
	...
	private void DisplayAttributes() {
		for(int cnt = 0; cnt < Enum. GetValues(typeof(AttributeName)).Length; cnt++) {
			...
			if(GUI.Button(new Rect(OFFSET + STAT_LABEL_WIDTH + BASEVALUE_LABEL_WIDTH + BUTTON_WIDTH, 40 + (cnt * BUTTON_HEIGHT), BUTTON_WIDTH, BUTTON_HEIGHT), "+")) {
				if(pointsLeft > 0) {
					_toon.GetPrimaryAttribute(cnt).BaseValue++;
					pointsLeft--;
					_toon.StatUpdate();
	...
	private void DisplaySkills() {
		for(int cnt = 0; cnt < Enum. GetValues(typeof(SkillName)).Length; cnt++) {
			...
			GUI.Label(new Rect((OFFSET * 3) + (STAT_LABEL_WIDTH * 2) + BASEVALUE_LABEL_WIDTH + (BUTTON_WIDTH *2), statStartingPos + (cnt * LINE_HEIGHT), BASEVALUE_LABEL_WIDTH, LINE_HEIGHT), _toon.GetSkill(cnt).AdjustedBaseValue.ToString() );	
		}
		
	}
	
	private void DisplayPointsLeft() {
		GUI.Label(new Rect((OFFSET * 2) + STAT_LABEL_WIDTH + (BASEVALUE_LABEL_WIDTH *2), (OFFSET * 2), STAT_LABEL_WIDTH, LINE_HEIGHT), "Points Left:  " + pointsLeft.ToString() );
	...
	private void DisplayCreateButton() {
		if(GUI.Button(new Rect(Screen.width/2 - 50, statStartingPos + (10 * LINE_HEIGHT), 100, LINE_HEIGHT), "Create")) {
			...
			// Change Cur Value of Vitals of Max Modified Value of that Vital
